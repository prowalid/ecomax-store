# فهرس التوثيق العربي

هذا الفهرس يوضح للمراجع الجديد أين يجد الحالة الحالية، وأين توجد المراجع التاريخية، وأين توجد الوثائق التشغيلية الخاصة بالنشر والمراقبة.

## المرجع الحالي

- [ARCHITECTURE_TRANSFORMATION_STATUS_AR.md](/root/express-trade-kit/docs/ARCHITECTURE_TRANSFORMATION_STATUS_AR.md)
- [STORE_PRODUCT_ARCHITECTURE_AR.md](/root/express-trade-kit/docs/STORE_PRODUCT_ARCHITECTURE_AR.md)
- [PARALLEL_STORE_HARDENING_PLAN_AR.md](/root/express-trade-kit/docs/PARALLEL_STORE_HARDENING_PLAN_AR.md)

## مراجع تاريخية

- [IMPLEMENTATION_STATUS_AR.md](/root/express-trade-kit/docs/IMPLEMENTATION_STATUS_AR.md)
- [PRODUCTION_ARCHITECTURE_ROADMAP_AR.md](/root/express-trade-kit/docs/PRODUCTION_ARCHITECTURE_ROADMAP_AR.md)
- [planb](/root/express-trade-kit/docs/planb)

## وثائق تشغيلية

- [CLOUD_DEPLOYMENT_CHECKLIST_AR.md](/root/express-trade-kit/deploy/CLOUD_DEPLOYMENT_CHECKLIST_AR.md)
- [CD_GITHUB_RUNBOOK_AR.md](/root/express-trade-kit/deploy/CD_GITHUB_RUNBOOK_AR.md)
- [SECRETS_AND_OPERATIONS_RUNBOOK_AR.md](/root/express-trade-kit/deploy/SECRETS_AND_OPERATIONS_RUNBOOK_AR.md)
- [GRAFANA_AR.md](/root/express-trade-kit/deploy/monitoring/GRAFANA_AR.md)
- [tests/load/README.md](/root/express-trade-kit/tests/load/README.md)

## ملفات النشر المهمة

- [releases.json](/root/express-trade-kit/deploy/releases.json)
- [docker-compose.client-stack.yml](/root/express-trade-kit/deploy/docker-compose.client-stack.yml)
- [docker-compose.monitoring.yml](/root/express-trade-kit/deploy/docker-compose.monitoring.yml)
- [client-multi.env.template](/root/express-trade-kit/deploy/client-multi.env.template)

## ملاحظة

إذا كنت تبحث عن صورة المشروع الحالية، ابدأ دائمًا من ملف الحالة المعمارية الحالي ثم انتقل إلى وثائق النشر والمراقبة حسب الحاجة.
