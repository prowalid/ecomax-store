...
- حالة إصدار رسمي.
- **إلزامي**: نفس رقم الإصدار للخدمتين (`api` و`web`) دون خلط.

### قاعدة توحيد الإصدار (Version Parity)

- ممنوع تشغيل `api` على Tag و`web` على Tag مختلف في نفس البيئة.
- المسموح فقط:
  - اختبار: كلاهما `stepdz-test`.
  - إصدار رسمي: كلاهما `vX.Y.Z` نفسه.
- أي حالة mixed تعتبر خطأ تشغيلي حتى لو كانت الحاويات تعمل.
...
- ممنوع خلط قناة اختبار مع قناة إصدار رسمي في نفس البيئة.
